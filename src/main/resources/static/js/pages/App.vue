<template>
    <v-app>
        <v-app-bar app color="primary" class="white--text">
            <v-toolbar-title style="cursor: pointer" @click="$router.push('/')">Shows App</v-toolbar-title>
            <v-spacer></v-spacer>
            <router-link to="/" class="mx-4 white--text">Bookings</router-link>
            <span v-if="$route.fullPath !== '/'"/>
            <span v-else-if="!profile">
                You're not authorized!
                <v-btn icon @click="$router.push('login')">
                    <v-icon style="color: #e7dede">login</v-icon>
                </v-btn>
            </span>
            <span v-else>
                {{ profile.login}}
